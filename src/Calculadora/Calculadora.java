		...
		do {
			...
			try {
				...
				if (opcionmenu > 0 && opcionmenu <= 7) { 	
					...
					String n2 = JOptionPane.showInputDialog("Introduce un número: [y] [M] [Pi] [E]\n\t[y] = el número que escojas o la base de la raíz/potencia/logaritmo\n\t[M] = el valor guardado en memoria: " + memoria + "\n\t[Pi] = " + Pi.substring(0, Math.min(Pi.length(), 6)) + "\n\t[E] = " + E.substring(0, Math.min(E.length(), 6)));
					    if (n2.equalsIgnoreCase("Pi")) {n2 = Pi;}
					    if (n2.equalsIgnoreCase("E")) {n2 = E;}
					    if (n2.equalsIgnoreCase("M") && memoria == null) {n2 = Double.toString(0);}
					    if (n2.equalsIgnoreCase("M") && memoria != null) {n2 = memoria;}
